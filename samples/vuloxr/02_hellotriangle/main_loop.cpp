...
public:
  Buffer(VkDevice device, const VkPhysicalDeviceMemoryProperties &props,
    ...
    if (vkCreateBuffer(device, &info, nullptr, &_buffer) != VK_SUCCESS) {
      vuloxr::Logger::Error("vkCreateBuffer");
      abort();
    }
    ...
    if (vkAllocateMemory(device, &alloc, nullptr, &_memory) != VK_SUCCESS) {
      vuloxr::Logger::Error("vkAllocateMemory");
      abort();
    }
...
void main_loop(const std::function<bool()> &runLoop,
  ...
  if (vkCreatePipelineCache(device, &pipelineCacheInfo, nullptr,
    ...
    vuloxr::Logger::Error("vkCreatePipelineCache");
    abort();
  }
...
#embed "triangle.frag"
  ...
  vuloxr::vk::CheckVkResult(vkCreateGraphicsPipelines(
      device, pipelineCache, 1, &pipe, nullptr, &pipeline));
