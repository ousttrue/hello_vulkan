...
template<class T> inline tmat3x4<T> to_mat3x4(const tmat4x4<T>& v)
{
    // copy with transpose
    return tmat3x4<T>{
...
template<class T> inline tmat4x4<T> to_mat4x4(const tmat3x4<T>& v)
{
    // copy with transpose
    return tmat4x4<T>{
        v[0][0], v[1][0], v[2][0], 0,
...
using uint = uint32_t;
inline int asint(float v) { return (int&)v; }
inline float asfloat(int v) { return (float&)v; }
