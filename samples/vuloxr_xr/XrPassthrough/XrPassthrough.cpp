...
void Egl::CreateContext(const Egl *shareEgl) {
  ...
  ALOGV("        Context = eglCreateContext( Display, Config, EGL_NO_CONTEXT, "
        "contextAttribs )");
  Context = eglCreateContext(Display, Config,
                             (shareEgl != nullptr) ? shareEgl->Context
                                                   : EGL_NO_CONTEXT,
                             contextAttribs);
  ...
  ALOGV("        TinySurface = eglCreatePbufferSurface( Display, Config, "
        "surfaceAttribs )");
...
void Egl::DestroyContext() {
  if (Display != 0) {
    ALOGE("        eglMakeCurrent( Display, EGL_NO_SURFACE, EGL_NO_SURFACE, "
          "EGL_NO_CONTEXT )");
  ...
  if (Context != EGL_NO_CONTEXT) {
    ...
    if (eglDestroyContext(Display, Context) == EGL_FALSE) {
      ALOGE("        eglDestroyContext() failed: %s",
            EglErrorString(eglGetError()));
    }
    Context = EGL_NO_CONTEXT;
  }
  if (TinySurface != EGL_NO_SURFACE) {
    ...
    if (eglDestroySurface(Display, TinySurface) == EGL_FALSE) {
      ALOGE("        eglDestroySurface() failed: %s",
            EglErrorString(eglGetError()));
    }
    TinySurface = EGL_NO_SURFACE;
  }
...
void Egl::DestroyContext() { ovrGl_DestroyContext_Windows(); }
...
void UpdateStageBounds(App &app) {
  ...
  app.StageBounds =
      Vector3f(stageBounds.width * 0.5f, 1.0f, stageBounds.height * 0.5f);
}
...
int main() {
  ...
  // Log available layers.
  {
    ...
    OXR(xrEnumerateApiLayerProperties(layerCount, &layerCount,
...
#endif // defined(XR_USE_PLATFORM_ANDROID)
      XR_FB_PASSTHROUGH_EXTENSION_NAME, XR_FB_TRIANGLE_MESH_EXTENSION_NAME};
  ...
  // runtime.
  {
    ...
    OXR(xrEnumerateInstanceExtensionProperties(nullptr, 0, &numOutputExtensions,
                                               nullptr));
    ALOGV("xrEnumerateInstanceExtensionProperties found %u extension(s).",
          numOutputExtensions);

    auto extensionProperties = std::vector<XrExtensionProperties>(
        numOutputExtensions, {XR_TYPE_EXTENSION_PROPERTIES});

    OXR(xrEnumerateInstanceExtensionProperties(NULL, numOutputExtensions,
                                               &numOutputExtensions,
  ...
  OXR(xrGetInstanceProcAddr(
      app.Instance, "xrGetOpenGLESGraphicsRequirementsKHR",
      (PFN_xrVoidFunction *)(&pfnGetOpenGLESGraphicsRequirementsKHR)));

  XrGraphicsRequirementsOpenGLESKHR graphicsRequirements = {
      XR_TYPE_GRAPHICS_REQUIREMENTS_OPENGL_ES_KHR};
  OXR(pfnGetOpenGLESGraphicsRequirementsKHR(app.Instance, systemId,
                                            &graphicsRequirements));
#elif defined(XR_USE_GRAPHICS_API_OPENGL)
  PFN_xrGetOpenGLGraphicsRequirementsKHR pfnGetOpenGLGraphicsRequirementsKHR =
      NULL;
  OXR(xrGetInstanceProcAddr(
      app.Instance, "xrGetOpenGLGraphicsRequirementsKHR",
      (PFN_xrVoidFunction *)(&pfnGetOpenGLGraphicsRequirementsKHR)));

  XrGraphicsRequirementsOpenGLKHR graphicsRequirements = {
      XR_TYPE_GRAPHICS_REQUIREMENTS_OPENGL_KHR};
  OXR(pfnGetOpenGLGraphicsRequirementsKHR(app.Instance, systemId,
                                          &graphicsRequirements));
#endif
  ...
  OXR(initResult =
          xrCreateSession(app.Instance, &sessionCreateInfo, &app.Session));
  if (initResult != XR_SUCCESS) {
  ...
  OXR(xrEnumerateViewConfigurations(app.Instance, app.SystemId, 0,
                                    &viewportConfigTypeCount, NULL));

  auto viewportConfigurationTypes =
      new XrViewConfigurationType[viewportConfigTypeCount];

  OXR(xrEnumerateViewConfigurations(
      app.Instance, app.SystemId, viewportConfigTypeCount,
  ...
  for (uint32_t i = 0; i < viewportConfigTypeCount; i++) {
    ...
    OXR(xrGetViewConfigurationProperties(app.Instance, app.SystemId,
    ...
    if (viewCount > 0) {
      ...
      OXR(xrEnumerateViewConfigurationViews(app.Instance, app.SystemId,
      ...
      for (uint32_t e = 0; e < viewCount; e++) {
        ...
        ALOGV("Viewport [%d]: Recommended Width=%d Height=%d SampleCount=%d", e,
  ...
  OXR(xrEnumerateReferenceSpaces(app.Session, numOutputSpaces, &numOutputSpaces,
                                 referenceSpaces));
  ...
  OXR(xrCreateSwapchain(app.Session, &swapChainCreateInfo,
                        &app.ColorSwapChain));
  OXR(xrEnumerateSwapchainImages(app.ColorSwapChain, 0, &app.SwapChainLength,
                                 nullptr));
#if defined(XR_USE_GRAPHICS_API_OPENGL_ES)
  ...
  OXR(xrEnumerateSwapchainImages(app.ColorSwapChain, app.SwapChainLength,
                                 &app.SwapChainLength,
  ...
  app.appRenderer.Create(format, width, height, NUM_MULTI_SAMPLES,
                         app.SwapChainLength, colorTextures);
  ...
  XrGeometryInstanceFB geomInstance = XR_NULL_HANDLE;
  {
    ...
    if (XR_SUCCEEDED(result)) {
      ...
      OXR(xrCreatePassthroughLayerFB(app.Session, &plci,
                                     &reconPassthroughLayer));
    }
    ...
    if (XR_SUCCEEDED(result)) {
      ...
      XrGeometryInstanceCreateInfoFB gici = {
...
#endif
  {
    ...
    for (;;) {
      ...
      const int timeoutMilliseconds =
          (app.Resumed == false && app.SessionActive == false &&
    ...
    XrPosef xfLocalFromHead;
    {
      ...
      OXR(xrLocateSpace(app.HeadSpace, app.LocalSpace,
    ...
    projectionInfo.viewConfigurationType =
    ...
    OXR(xrLocateViews(app.Session, &projectionInfo, &viewState,
    ...
    for (int i = 0; i < 4; i++) {
      if (controllerActive[i >> 1]) {
        XrSpaceLocation loc = {XR_TYPE_SPACE_LOCATION};
        OXR(xrLocateSpace(controllerSpace[i], app.LocalSpace,
                          frameState.predictedDisplayTime, &loc));
    ...
    const Mode prevMode = Mode(((frameCount - framesCyclePaused +
                                 (framesPerMode * Mode_NumModes) - 1) /
                                framesPerMode) %
                               Mode_NumModes);
    const Mode mode = Mode(((frameCount - framesCyclePaused) / framesPerMode) %
                           Mode_NumModes);
    ...
    if (mode == Mode_Passthrough_DynamicRamp) {
      const float frac = ((frameCount - framesCyclePaused) % framesPerMode) /
                         (framesPerMode - 1.0f);
      ...
      style.next = &colorMap;
      OXR(xrPassthroughLayerSetStyleFB(passthroughLayer, &style));
    } else if (mode == Mode_Passthrough_ProjQuad &&
               leftControllerGripSpace != XR_NULL_HANDLE) {
      XrGeometryInstanceTransformFB git = {
    ...
    if (app.StageSpace != XR_NULL_HANDLE) {
      XrSpaceLocation loc = {XR_TYPE_SPACE_LOCATION};
      OXR(xrLocateSpace(app.StageSpace, app.LocalSpace,
                        frameState.predictedDisplayTime, &loc));
    ...
    while (res == XR_TIMEOUT_EXPIRED) {
      ...
      retry++;
      ALOGV(" Retry xrWaitSwapchainImage %d times due to XR_TIMEOUT_EXPIRED "
            "(duration %f seconds)",
            retry, waitInfo.timeout * (1E-9));
    }
