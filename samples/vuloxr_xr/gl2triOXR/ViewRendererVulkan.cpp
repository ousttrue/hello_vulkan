...
struct Impl {
  ...
  void initScene(const char *vertexShaderGlsl, const char *fragmentShaderGlsl,
                 std::span<const VertexAttributeLayout> layouts, const void *pV,
                 uint32_t vertexCount) {

    uint32_t stride = 0;
    for (auto &l : layouts) {
      stride += l.components * 4;
    }

    for (uint32_t i = 0; i < layouts.size(); ++i) {
      VkFormat format;
      switch (layouts[i].components) {
      case 2:
      ...
      this->vertices.attributes.push_back({
          ...
          .binding = 0,
          .format = format,
          .offset = static_cast<uint32_t>(layouts[i].offset),
      });
    }
    ...
    this->vertices.allocate(this->graphics->physicalDevice,
                            this->graphics->device, pV, stride * vertexCount,
                            vertexCount);
...
void ViewRenderer::initScene(const char *vs, const char *fs,
  ...
  _impl->initScene(vs, fs, layouts, vertices, vertexCount);
}
