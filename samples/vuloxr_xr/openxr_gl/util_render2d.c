...
static char *s_shader[SHADER_NUM * 2] = {
    vs_fill, fs_fill,     vs_tex,      fs_tex,      vs_tex,      fs_extex,
    vs_tex,  fs_cmap_jet, vs_tex_yuyv, fs_tex_yuyv, vs_tex_uyvy, fs_tex_uyvy,
};
...
int draw_2d_texture_ex(texture_2d_t *tex, int x, int y, int w, int h,
                       int upsidedown) {
  texparam_t tparam = {0};
  tparam.x = x;
  tparam.y = y;
...
int draw_2d_texture_texcoord(int texid, int x, int y, int w, int h,
                             float *user_texcoord) {
  texparam_t tparam = {0};
  tparam.x = x;
  tparam.y = y;
...
int draw_2d_texture_ex_texcoord(texture_2d_t *tex, int x, int y, int w, int h,
                                float *user_texcoord) {
  texparam_t tparam = {0};
  tparam.x = x;
  tparam.y = y;
...
int draw_2d_texture_ex_texcoord_rot(texture_2d_t *tex, int x, int y, int w,
                                    int h, float *user_texcoord, float px,
                                    float py, float deg) {
  texparam_t tparam = {0};
  tparam.x = x;
  tparam.y = y;
...
int draw_2d_colormap(int texid, int x, int y, int w, int h, float alpha,
                     int upsidedown) {
  texparam_t tparam = {0};
  tparam.x = x;
  tparam.y = y;
...
int draw_2d_rect(int x, int y, int w, int h, float *color, float line_width) {
  ...
  float y2 = y + h;
  if (sobj->loc_vtx >= 0) {
    float vtx[] = {x1, y1, x2, y1, x2, y2, x1, y2, x1, y1};
...
int draw_2d_rect_rot(int x, int y, int w, int h, float *color, float line_width,
  ...
  float y2 = y + h;
  if (sobj->loc_vtx >= 0) {
    float vtx[10] = {x1, y1, x2, y1, x2, y2, x1, y2, x1, y1};
...
int draw_2d_line(int x0, int y0, int x1, int y1, float *color,
                 float line_width) {
  if (line_width == 1.0f) {
    int ttype = 0;
    shader_obj_t *sobj = &s_sobj[ttype];
...
int draw_2d_circle(int x, int y, int radius, int ndiv, float *color,
                   float line_width) {
  int ttype = SHADER_TYPE_FILL;
  shader_obj_t *sobj = &s_sobj[ttype];
  float matrix[16];
  ...
  if (sobj->loc_vtx >= 0) {
    ...
    glEnableVertexAttribArray(sobj->loc_vtx);
    glVertexAttribPointer(sobj->loc_vtx, 2, GL_FLOAT, GL_FALSE, 0, vtx);
    glDrawArrays(GL_LINE_STRIP, 0, ndiv + 1);
  }
  glLineWidth(1);
