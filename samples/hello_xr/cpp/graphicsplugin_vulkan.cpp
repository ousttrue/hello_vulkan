...
#define CHECK_CBSTATE(s)                                                       \
  ...
  bool Init(const VulkanDebugObjectNamer &namer, VkDevice device,
...
private:
  ...
  void Load(uint32_t index, const std::vector<uint32_t> &code) {
    ...
    CHECK_VKCMD(
        vkCreateShaderModule(m_vkDevice, &modInfo, nullptr, &si.module));
...
struct DepthBuffer {
  ...
  void Create(const VulkanDebugObjectNamer &namer, VkDevice device,
    ...
    CHECK_VKCMD(namer.SetName(VK_OBJECT_TYPE_DEVICE_MEMORY,
                              (uint64_t)depthMemory,
...
// adjust the window size and show at InitDevice time
  ...
  CHECK_VKCMD(vkCreateWin32SurfaceKHR(m_vkInstance, &surfCreateInfo, nullptr,
                                      &surface));
#else
...
#error CreateSurface not supported on this OS
    ...
    if (vkCreateDebugUtilsMessengerEXT != nullptr) {
      CHECK_VKCMD(vkCreateDebugUtilsMessengerEXT(
          m_vkInstance, &debugInfo, nullptr, &m_vkDebugUtilsMessenger));
    }
...
#include "frag.spv"
    ...
    CHECK_VKCMD(
        vkCreateSemaphore(m_vkDevice, &semInfo, nullptr, &m_vkDrawDone));
    CHECK_VKCMD(m_namer.SetName(VK_OBJECT_TYPE_SEMAPHORE,
                                (uint64_t)m_vkDrawDone,
                                "hello_xr draw done semaphore"));
