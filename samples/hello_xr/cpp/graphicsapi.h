...
struct IGraphicsApi {
  ...
  virtual std::shared_ptr<IGraphicsAdapter>
  CreateAdapter(XrInstance instance, XrSystemId systemId) = 0;

  // OpenXR extensions required by this graphics API.
  virtual std::vector<std::string> GetInstanceExtensions() const = 0;
};

std::shared_ptr<IGraphicsApi>
CreateGraphicsAPI(const std::shared_ptr<struct Options> &options,
