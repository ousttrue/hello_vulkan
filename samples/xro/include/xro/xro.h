// based on hello_xr
//
// simplified that use only XR_TYPE_GRAPHICS_REQUIREMENTS_VULKAN2_KHR
#pragma once
#include <vko/vko.h>
...
struct Swapchain {
  ...
  Swapchain(XrSession session, uint32_t i, const XrViewConfigurationView &vp,
    ...
    XRO_CHECK(xrCreateSwapchain(session, &this->swapchainCreateInfo,
                                &this->swapchain));
    ...
    XRO_CHECK(xrEnumerateSwapchainImages(this->swapchain, imageCount,
  ...
  AcquireSwapchain(const XrView &view) {
    ...
    XRO_CHECK(xrAcquireSwapchainImage(this->swapchain, &acquireInfo,
                                      &swapchainImageIndex));
    ...
    XRO_CHECK(xrWaitSwapchainImage(this->swapchain, &waitInfo));
  ...
  void EndSwapchain() {
    ...
    XRO_CHECK(xrReleaseSwapchainImage(this->swapchain, &releaseInfo));
  }
