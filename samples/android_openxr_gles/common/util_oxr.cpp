...
oxr_alloc_swapchain_rtargets(XrSwapchain swapchain, uint32_t width,
  ...
  for (uint32_t i = 0; i < imgCnt; i++) {
    ...
    LOGI("SwapchainImage[%d/%d] FBO:%d, TEXC:%d, TEXZ:%d, WH(%d, %d)", i,
         imgCnt, fbo, tex_c, tex_z, width, height);
  }
  free(img_gles);
  return 0;
}
...
XrHandJointLocationsEXT *oxr_create_handjoint_loc() {
  ...
  XrHandJointVelocityEXT *vel_data =
      new XrHandJointVelocityEXT[XR_HAND_JOINT_COUNT_EXT]{};
  XrHandJointLocationsEXT *loc = new XrHandJointLocationsEXT{};
  XrHandJointLocationEXT *loc_data =
      new XrHandJointLocationEXT[XR_HAND_JOINT_COUNT_EXT]{};
    ...
    for (int i = 0; i < XR_HAND_JOINT_COUNT_EXT; i++) {
      if ((loc_array[i].locationFlags & isValid)) {
        ...
        LOGI("Joint[%d/%d]: POS(%f, %f, %f), RAD(%f)\n", i,
             XR_HAND_JOINT_COUNT_EXT, pose.position.x, pose.position.y,
             pose.position.z, rad);
      }
    }
  } else {
...
int oxr_resume_passthrough_layer(XrInstance instance,
                                 XrPassthroughLayerFB ptLayer) {
